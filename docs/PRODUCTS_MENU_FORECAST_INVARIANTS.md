...
| Interpretation | Variant multipliers, bundle composition, and mapped modifiers affect physical units according to canonical interpretation. A sale of a bundle is one bundle unit and each member's count, recursively; sales-accounting units remain per product. The production plan has a separately labelled aggregate count across planned product rows, never a sales total. | Sales interpretation; bundle/modifier/refund tests. |
...
| Identity and scope | Production totals sum displayed product rows, including bundle/modifier demand. Basis, series and accuracy compare menu members only, including unpriced members. Neither restates as-sold accounting. | Bundle/unpriced cross-product tests; chart and basis labels explicitly name their scope. |
